...
//------------------------------------------------------------------
void particle::update(float speed, float noise, int lenghtPoints) {
	...
	//	frc.y = ofSignedNoise(uniqueVal, pos.x * 0.006, ofGetElapsedTimef() * 0.2) * 0.09 + 0.18;
		...
		vel -= repelFrc;// *0.0001; //+attract the point 
...
void particle::repel(vector<ofVec2f>repelPt){
	...
	for (int i = 0; i < repelPt.size(); i++) {
		//glm::vec3 attractPt(ofGetMouseX(), ofGetMouseY(), 0);
		float dist = pos.distance(repelPt[i]);//ofDist(repelPt[i].x, repelPt[i].y, pos.x,pos.y);
		
		if (dist < 150) {

			ofVec2f newRepelPt;
			newRepelPt = repelPt[i] - pos; // we get the attraction force/vector by looking at the mouse pos relative to our pos
			repelFrc += newRepelPt*0.0001;// .normalized(); //by normalizing we disregard how close the particle is to the attraction point
		...
		//ofVec2f ptDist; 
		//ptDist = repelPt[i] - pos; // we get the attraction force/vector by looking at the mouse pos relative to our pos
		//float dist = ofDist(repelPt[i].x, repelPt[i].y, pos.x,pos.y);
		//ofVec2f addedRepelFrc;
		//addedRepelFrc.x = ofMap(dist, 0, 200, 0.01, 0, true);
		//addedRepelFrc.y = ofMap(dist, 0, 200, 0.01, 0, true);
		
		//repelFrc += addedRepelFrc;// .normalized(); //by normalizing we disregard how close the particle is to the attraction point
...
void particle::draw(float sizeDot) {
	...
	for (int i = 0; i < trail.size(); i++) {
		ofDrawCircle(trail[i].x, trail[i].y, ofMap(i,0, trail.size(),scale * sizeDot*0.5, scale * sizeDot));
	}
}
